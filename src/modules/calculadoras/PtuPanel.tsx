...
export default function PtuPanel() {
  ...
  useEffect(() => {
    ...
    Promise.all([
      supabase
        .from('empleados')
        .select('id, nombre, apellido_paterno, codigo, fecha_ingreso, fecha_baja, empresa_id, estatus'),
  ...
  const elegibles = useMemo(
    () =>
      empleados
        .filter((e) => e.dias_trabajados >= 60)
        .filter((e) => !empresaId || e.empresa_id === empresaId),
    [empleados, empresaId],
  );
    ...
    const tbody = filas
      .map(
        (r) => `<tr>
          ...
          <td class="r">${r.dias_trabajados}</td>
          <td class="r">${fmt.format(r.sueldo)}</td>
          <td class="r">${fmt.format(r.porDias)}</td>
          <td class="r">${fmt.format(r.porSalario)}</td>
          <td class="r"><b>${fmt.format(r.total)}</b></td>
        </tr>`,
      )
      .join('');
    const html = `<!doctype html><html><head><meta charset="utf-8">
<title>PTU ${anio}</title>
<style>
...
${pdfHeaderHTML(empresa, `PTU ${anio}`, `${filas.length} elegibles · Monto a repartir ${fmt.format(montoTotal)}`)}
<table>
  <thead><tr>
    <th>Código</th><th>Empleado</th>
    <th class="r">Días</th><th class="r">Sueldo</th>
    <th class="r">Por días</th><th class="r">Por salario</th><th class="r">Total PTU</th>
  </tr></thead>
  <tbody>${tbody}</tbody>
  <tfoot><tr class="totales">
    <td colspan="6" style="text-align:right;padding:8px">TOTAL REPARTIDO</td>
    <td class="r" style="padding:8px">${fmt.format(filas.reduce((a, f) => a + f.total, 0))}</td>
  </tr></tfoot>
</table>
    ...
    <div className="flex flex-col gap-4">
      ...
      <div className="grid grid-cols-1 gap-3 rounded-lg border border-slate-200 bg-white p-4 md:grid-cols-4">
        ...
        <div className="flex items-end justify-end gap-2">
          <Button variant="secondary" size="sm" onClick={csv} disabled={!montoTotal}>
            <Download size={14} /> CSV
          </Button>
          <Button variant="secondary" size="sm" onClick={pdf} disabled={!montoTotal}>
